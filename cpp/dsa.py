...
def sum_of_squares(data: int): 
    return sum([x**2 for x in range(max(1, data + 1))])

def sum_of_squares_odd(data: int):
    return sum([x**2 for x in range(1,max(1, data+1),2 )])
...
if __name__ == "__main__":
    ...
    for c in range(3):
        ...
        print("Lijmit = ", wallet[c].get_balance())
        while wallet[c].get_balance() > 100:
            wallet[c].make_payment(100)
            print("new blanace =", wallet[c].get_balance)